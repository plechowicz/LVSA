...
public class FileChooser extends JFileChooser {
	 ...
	 * <p>
	 * At the beginning when no file is choose or errors occurs while opening a
	 * file the file state takes NO_FILE. If file is currently open and ready
	 * for actions the file state takes FILE_OPEN. If file is saving, file state
	 * takes FILE_SAVE to assured that no one is interrupting this.
	 */
	enum FileState {
	...
	/**
	 * Open a File with JFileChooser If file is selected it is send to
	 * audioFileOpenerAndSaver to convert its format State of the file is stored
	 * in fileState.
	 */
	...
	public void approveSelection() {
		...
		if (f.exists() && getDialogType() == SAVE_DIALOG) {
			int result = JOptionPane.showConfirmDialog(this, "The file exsists, overwrite?", "Exsisting file",
	...
	/**
	 * Save audio byte data. Value of fileState and filePathToOpen depends on
	 * errors. If File is successfully saved fileState gets value FILE_OPEN, and
	 * filePathToOpen gets value the same as filePathToSave. If errors occurred,
	 * fileState gets value FILE_OPEN if there was opened file before, but the
	 * filePathToOpen will not change its value. If previously no file were
	 * opened, fileState gets value NO_FILE.
	 * 
	 * @param data
	 */
	 ...
	 * files with different format that given in class Utils. The supported file
	 * formats are WAVE, AIFF, AU.
	 */
	class AudioFileFilter extends FileFilter {
		...
		public boolean accept(File f) {
			...
			if (extension != null) {
				if (extension.equals(Utils.wav) || (extension.equals(Utils.wave)) || extension.equals(Utils.aif)
						|| extension.equals(Utils.aiff) || extension.equals(Utils.aifc) || extension.equals(Utils.au)
