...
public class ButtonListener {
	...
	/**
	 * AudioPanelButtonsAndFields auto-reference for changing the availability
	 * of buttons.
	 */
	...
	private DisplayPanel displayPanelWaveform, displayPanelAnalysis;
	...
	/******************************************************************************************
	 * 
	 * Klasy b�d�ce odpowiednimi Listenerami
	 * 
	 * *************************************************************************
	 * ***************
	 */
	...
	/**
	 * Action Listener for open file button. It acts differently whether the
	 * file opening finish with success or not.
	 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (fileChooser.getFileState() == FileChooser.FileState.FILE_OPEN) {
				...
				synchronized (audioLock) {
					audioFloatArray = AudioFileOperations
							.byteArrayIntoFloatArray(AudioFileOperations.getByteArrayWithAudio());
				}
	 ...
	 * save audio bytes array into chose directory. Open that file and set text
	 * field with its name.
	 */
	class BtnSaveFileListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (fileChooser.getFileState() == FileChooser.FileState.FILE_SAVE) {
				...
				if (fileChooser.getFileState() == FileChooser.FileState.FILE_OPEN) {
					fileName = fileChooser.getFilePathToOpen();
					// buttons.textFieldFileName.setText(name.substring(name.lastIndexOf('\\')
					// + 1));
					setNameInTextField(fileName);
				}
			}
	...
	class BtnPRPauseListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			Debug.debug("pause");
			if (buttons.pauseFile.isAvaliable())
				buttons.pauseFile.changeState();
			player.playback.pause();
			if (!buttons.playFile.isAvaliable())
				buttons.playFile.changeState();
		}
	}
	...
	class StartAnalysisThread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				analysisDataWindow.addAnalysisData(fileName, getFrequencyFromTheTextField(),
						getDecisionFromTheTextField());
	...
	/*****************************************************************************
	 * 
	 * Zmiany stan�w przycisk�w i p�l
	 * 
	 * *************************************************************************
	 * **
	 */
	...
	private void setFrequencyInTheTextField(float frequency) {
		...
		int i;
		text = (((i = text.indexOf('.')) != -1) ? text.substring(0, (i + 3 < text.length() ? i + 3 : i + 2)) + " Hz"
				: text + " HZ");
	...
	void buttonsStatePlayFile() {
		synchronized (this) {
			if (buttons.playFile.isAvaliable())
				buttons.playFile.changeState();
			if (buttons.openFile.isAvaliable())
				buttons.openFile.changeState();
			if (buttons.saveFile.isAvaliable())
				buttons.saveFile.changeState();
			if (!buttons.pauseFile.isAvaliable())
				buttons.pauseFile.changeState();
			if (!buttons.stopFile.isAvaliable())
				buttons.stopFile.changeState();
			if (buttons.recordFile.isAvaliable())
				buttons.recordFile.changeState();
	...
	void buttonsStateRecordFile() {
		synchronized (this) {
			...
			if (buttons.recordFile.isAvaliable())
				buttons.recordFile.changeState();
			if (buttons.startAnalysis.isAvaliable())
				buttons.startAnalysis.changeState();
			if (buttons.stopAnalysis.isAvaliable())
				buttons.stopAnalysis.changeState();
			buttons.menuItemMixerChooser.setEnabled(false);
		}
	}
	 ...
	 * 
	 * @param frame
	 *            JFrame on which Button Listeners listens out actions
	 */
	...
	/**
	 * System will exit after saving a file. It will wait for end of that
	 * operation.
	 */
