...
public class AnalysisDataWindow implements ActionListener {
			...
			class SaveTxtFileChooser extends JFileChooser {
				...
				public void approveSelection() {
					...
					if (f.exists() && getDialogType() == SAVE_DIALOG) {
						int result = JOptionPane.showConfirmDialog(this, "The file exsist, overwrite?",
								"Exsisting file", JOptionPane.YES_NO_OPTION);
						switch (result) {
	...
	/**
	 * Stworzenie nag��wka dla kolumn.
	 * 
	 * @return ci�g znak�w umieszczany w nag��wku
	 */
	...
	/**
	 * Dodanie kolejnego wiersza do pola przechowuj�cego wyniki wykonanych
	 * analiz.
	 * 
	 * @param fileName
	 *            nazwa pliku
	...
	public void addAnalysisData(String fileName, String frequency, String decision) {
		...
		if (fileName.length() > spaceForFileName - 2)
			name = fileName.substring(0, 3) + "..."
					+ fileName.substring(fileName.length() - (spaceForFileName - 9), fileName.length()) + "  ";
		else {
			name = fileName;
