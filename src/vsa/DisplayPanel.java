...
public class DisplayPanel extends JPanel {
	...
	// kazdy sampel to jedno miejsce do narysowania bo sygnal jest mono, wi�c
	// nie trzeba nic u�rednia�
		...
		synchronized (stringLock) {
			if (values != null) {
				...
				if (orderOfMagnitude >= 0) {
					...
					g2d.drawString(values[0].substring(0, values[0].length() - 2), 2,
							this.getSize().height - 2 - fontHeight);
					g2d.drawString(values[0].substring(values[0].length() - 2), 2, this.getSize().height - 2);
					for (int i = 1; i < values.length; i++) {
						g2d.drawString(values[i].substring(0, values[i].length() - 2), yAxes[i - 1] + 2,
								this.getSize().height - 2 - fontHeight);
						g2d.drawString(values[i].substring(values[i].length() - 2), yAxes[i - 1] + 2,
