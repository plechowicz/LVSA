...
/**
 * State that describes if file is currently
 * playing, if it has been stopped, paused, resumed, or it is recording. 
 */
enum PlayState {
...
public class Player {
	...
	/**
	 * Is audio file playing. Describes if there is a thread for playing or capturing audio running.
	 */
	...
	class Playback implements Runnable {
		...
		/**
		 * Start playing audio. If file is stopped create a new thread and start from beginning,
		 * if file were paused resume from that place.
		 */
		void play() {
		...
		/**
		 * Pause audio file. It is possible only when file were playing or resumed.
		 */
		void pause() {
		...
		/**
		 * If errors occurred while playing audio, shut down the thread. PlayState gets value STOP
		 * and in override method run() play state is checked to stop the thread.
		 * @param message message that will show in pop up dialog
		 */
		...
		public void run() {
			...
			//przerwij je¿eli oka¿e siê, ¿e PlayState ma wartoœæ STOP
		...
		/**
		 * Get source data line to send audio to speakers.
		 * First defines the data line info to check if that line is supported in the operating system.
		 * Then try to obtain that line from audio system. 
		 */
		private void getSourceDataLine() {
			...
			if (MixerChooser.getOutputMixerState() == MixerChooser.MixerState.DEFAULT) {
		    	...
		    	// pobierz i otwórz source data line do odtwarzania.
		...
		private void playAudio() {
			...
			//byte array used for passing data from input stream to source data line
			...
			while(((playState == PlayState.PLAY ) || (playState == PlayState.PAUSE) || (playState == PlayState.RESUME)) && (thread != null)) {
				// if play state is pause wait for resume (resume will do notifyAll())
			...
			// It is necessary to switch buttons (like there were pressed stop button).
			if (playState != PlayState.STOP) {
	...
	class Capture implements Runnable {
		...
		/**
		 * Start recording audio. It is possible only when there is no audio playing at this moment.
		 */
		void start(){
			if (playState == PlayState.STOP) {
		...
		/**
		 * If errors occurred while recording audio, shut down the thread. PlayState gets value STOP
		 * and in override method run() play state is checked to stop the thread.
		 * @param message message that will show in pop up dialog
		 */
		private void shutDown(String message) {
		...
		/**
		 * Get target data line to capture audio from microphone.
		 * First defines the data line info to check if that line is supported in the operating system.
		 * Then try to obtain that line from audio system. 
		 */
